param(
    ...
    [string] $Hash = "ea3d24bbe3c58b171e55fe2151fc7ffaca3ab3d2",
    [array] $Patches = @(
        @{
            PatchFile = "${PSScriptRoot}/patches/FFmpeg/0001-FFmpeg-6.0-OBS-Windows.patch"
            HashSum = "74c5f57ce73f743c093adee82450d09f8f817c418cba9a50aeda994a03de6ee0"
        }
    ),
    [array] $Targets = @('x64')
)
...
function Setup {
    ...
    if ( ! ( $SkipAll -or $SkipDeps ) ) {
        Invoke-External pacman.exe -S --noconfirm --needed --noprogressbar yasm
...
function Configure {
    ...
    $ConfigureCommand = @(
        ...
        '--toolchain=msvc'
        ('--extra-cflags=' + "'-D_WINDLL -MD -DWINAPI_FAMILY=WINAPI_FAMILY_APP -D_WIN32_WINNT=0x0A00'")
        ('--extra-cxxflags=' + "'-MD -DWINAPI_FAMILY=WINAPI_FAMILY_APP -D_WIN32_WINNT=0x0A00'")
    ...
    ($(Get-Content config.h) -replace '[^\x20-\x7D]+', '') > config.h
}
